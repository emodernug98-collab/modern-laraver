...
function suggestionSearchText(item: SearchSuggestion) {
  return [
    item.title,
    item.slug,
...
export default function NavBar({
  ...
  const groupedCategories = useMemo(() => {
    ...
    // Hide specific titles from the mega menu (matched on title for now —
    // could be promoted to a backend "hidden_in_nav" flag later).
    const excludedTitles = ["ACs & Air Coolers", "Electronics Lab Tools"];

    const all = data.categories.filter(
      (c) => c.isActive && !excludedTitles.includes(c.title)
    );

    // A category is a root when parentId is null/empty. Falls back to the
    // legacy `rootCategory` string check only when parentId isn't exposed
    // (older payloads from a not-yet-updated backend).
    const isRoot = (c: typeof all[number]) => {
      if (c.parentId === undefined) return !c.rootCategory;
      return c.parentId === null || c.parentId === "";
    };

    const roots = all.filter(isRoot);
    const subCats = all.filter((c) => !isRoot(c));
        ...
        subcategories: subCats
          .filter((child) =>
            child.parentId
              ...
              : child.rootCategory === root.title
          )
          // Drop orphans whose parent was filtered out
          .filter((child) => !child.parentId || byId.has(child.parentId)),
      }))
  ...
  const allSearchSuggestions = useMemo(() => {
    ...
    for (const category of data.categories.filter((item) => item.isActive)) {
      suggestionsById.set(`category-${category.id}`, {
        ...
        slug: category.slug,
        searchKeywords: [category.rootCategory || ""],
      });
    }
  ...
  const searchTerms = useMemo(() => {
    ...
    const baseTerms = allSearchSuggestions
      .filter((item) =>
        normalizedQuery ? suggestionSearchText(item).includes(normalizedQuery) : true
      )
      .slice(0, 6)
      .map((item) => item.title);
  ...
  const visibleSuggestions = useMemo(() => {
    ...
    return allSearchSuggestions.filter((item) =>
      suggestionSearchText(item).includes(normalizedQuery)
    ).slice(0, 10);
        ...
        setBackendSearchSuggestions(
          (payload.products ?? [])
            .map((product) => ({
              ...
              slug: product.slug || slugFromHref(product.href || ""),
              image: product.image || "",
              href: product.href || "",
              searchKeywords: [product.slug || ""],
            }))
